...
hyneiji = sgs.General(extension, "hyneiji", "wu", "3", true, true, true)
...
hyfenshen = sgs.CreateViewAsSkill{
	name = "hyfenshen",
...
hyfenshenHid = sgs.CreateTargetModSkill{
	name = "#hyfenshenHid",
	pattern = "Slash",
		...
		if player:isWounded() then
			if not player:isKongcheng() then
				if player:askForSkillInvoke("hyyinghua", data) then
					local room = player:getRoom()
					if room:askForDiscard(player, self:objectName(), 1, 1, true) then
						local recover = sgs.RecoverStruct()
						recover.who = player
						room:recover(player, recover)
					end
...
hybadun = sgs.CreateTriggerSkill{
	...
	events = {sgs.DamageCaused,sgs.ConfirmDamage,sgs.EventPhaseStart},
	on_trigger = function(self, event, player, data)
...
hynaruto:addSkill(hyfenshenHid)
...
sgs.LoadTranslationTable{
	...
	["hyzhangxianVS"] = "掌仙",
	["hyzhangxian"] = "掌仙",
	[":hyzhangxian"] = "你可以弃置一张【桃】并指定一名没有【樱】标记的角色，该角色获得一枚【樱】标记。拥有【樱】标记的角色，在其下个回合开始前，则须选择回复一点体力或增加一点体力上限，然后该角色弃置【樱】标记。",
	["@sakura"] = "樱",
	...
	["hybadun"] = "八遁",
	[":hybadun"] = "<b>锁定技</b>,出牌阶段，每当你使用一张【杀】造成伤害，你获得一枚【门】标记（最多8枚）。任何时候，你的【杀】造成的伤害始终+X（X=【门】标记的数量）。回合结束阶段开始时，若【门】标记数量为8枚，你立即死亡。",
	["@men"] = "门",
	["hyliandanCard"] = "连弹",
	["hyliandanVS"] = "连弹",
	["hyliandan"] = "连弹",
	[":hyliandan"] = "出牌阶段，你可以失去1点体力并指定一名角色，你与该角色的距离始终视为1，直到回合结束。每阶段限1次。",
	["hyroubo"] = "肉搏",
